      ...
      if (res['return'] != 0) {
        if (this.common.IsMobile()) {//手机跳转页面
          ...
          eleButton.onclick = function () {
            $message({ message: '浏览器不支持', type: 'warning' })
          }
        }
      }
      else {
        $message({ message: '请求错误，请确认链接地址是否正确', type: 'warning' })
      }
    },
    getSpeech () {
      if (this.form.url == '') {
        $message({ message: '你还没添加链接呢！', type: 'warning' })
      }
      else {
        this.api.GetSpeech({ url: this.form.url }).then(res => {
          ...
          $message({ message: '输入格式不正确', type: 'Error' })
        })
      }
    }
  },
