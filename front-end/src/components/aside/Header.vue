  ...
  <div class="my-header-aside">
    <el-collapse accordion>
      <el-collapse-item>
        ...
        <ul class="my-header-aside-ul">
          <router-link tag="li" :to="'/subindex/' + cate.id"
            v-for="cate in category"
            :id="cate.id"
            :key="cate.id"
            :index="cate.id.toString()"
            :class="cate.id == routerId?'my-li-active':''"
          >
            <i :class="'el-icon-blog-' + cate.CategoryName"></i>
...
export default {
  ...
  computed: {
    routerId: function () {
      ...
      if(this.$route.name === 'article') return -1;
